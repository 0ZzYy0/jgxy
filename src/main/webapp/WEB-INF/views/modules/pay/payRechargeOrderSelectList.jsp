	...
	<div class="wrapper-content">
		<div class="ibox">
			...
			<sys:message content="${message}"/>
				...
				<div class="row">
					<div class="col-sm-12">
						<form:form id="searchForm" modelAttribute="payRechargeOrder" action="${ctx}/pay/payRechargeOrder/selectList" method="post" class="form-inline">
							...
							<table:sortColumn id="orderBy" name="orderBy" value="${page.orderBy}" callback="sortOrRefresh();"/><!-- 支持排序 -->
								...
								<div class="form-group"><span>创建时间：</span>
									<input name="beginCreateDate" type="text" readonly="readonly" maxlength="20" class="laydate-icon form-control input-medium"
										value="<fmt:formatDate value="${payRechargeOrder.beginCreateDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
									...
									<input name="endCreateDate" type="text" readonly="readonly" maxlength="20" class="laydate-icon form-control input-medium"
										value="<fmt:formatDate value="${payRechargeOrder.endCreateDate}" pattern="yyyy-MM-dd HH:mm:ss"/>"
										onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',isShowClear:false});"/>
								</div>
						</form:form>
						<br>
				...
				<table id="contentTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
					...
					<c:forEach items="${page.list}" var="payRechargeOrder">
						<tr>
							...
							<td class="codelabel">${payRechargeOrder.no}</td>
							<td>${payRechargeOrder.amount}</td>
							<td>${fns:getDictLabel(payRechargeOrder.status, 'yes_no', '')}</td>
							<td>${fns:getDictLabel(payRechargeOrder.payType, 'pay_type', '')}</td>
							<td><fmt:formatDate value="${payRechargeOrder.createDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
							<td>${payRechargeOrder.createBy.id}</td>
							<td>
