	...
	<div class="wrapper-content">
		<div class="ibox">
			...
			<sys:message content="${message}"/>
				...
				<div class="row">
					<div class="col-sm-12">
						<form:form id="searchForm" modelAttribute="genReport" action="${ctx}/gen/genReport/selectList" method="post" class="form-inline">
				...
				<table id="contentTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
					<thead>
						<tr>
							...
							<th class="sort-column a.table_name">表名</th>
							<th class="sort-column a.report_type">图表类型</th>
					...
					<c:forEach items="${page.list}" var="genReport">
						<tr>
							...
							<td class="codelabel">${genReport.name}</td>
							<td>${genReport.comments}</td>
							<td>${genReport.tableName}</td>
							<td>${fns:getDictLabel(genReport.reportType, 'report_type', '')}</td>
							<td>${genReport.createBy.id}</td>
							<td><fmt:formatDate value="${genReport.createDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
							<td>
								<shiro:hasPermission name="gen:genReport:view">
									<a href="#" onclick="openDialogView('查看图表配置', '${ctx}/gen/genReport/view?id=${genReport.id}','800px', '500px')" class="btn btn-info btn-xs" title="查看"><i class="fa fa-search-plus"></i> 查看</a>
								</shiro:hasPermission>
							</td>
						</tr>
					</c:forEach>
