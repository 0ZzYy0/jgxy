        	...
        	<div class="col-sm-2">
                <div class="ibox float-e-margins">
                    ...
                    <div class="ibox-content">
                    	...
                    	<div class="form-group">
                    		 <button type="button" class="btn btn-primary btn-circle m-r-sm" onclick="javascript:window.location.href='${ctx}/wms/wmsOutstock/list?status=0'"><i class="fa fa-file-o"></i></button>
                                    ...
                                    <div class="media-body ">
                                    	<span class="pull-right">
											...
											<shiro:hasPermission name="wms:wmsInstock:audit">	
												<a href="${ctx}/wms/wmsInstock/audit?id=${wmsInstock.id}" onclick="return confirmx('确认要审核该入库单吗？', this.href)" class="btn btn-primary btn-sm" title="审核"><i class="fa fa-check"></i> 审核</a>
											</shiro:hasPermission>
											</c:if>
                                        </span>
                                        <strong>单号：${wmsInstock.no}</strong>，内容：${wmsInstock.content}
                                        <p class="m-b-xs">
                                            <i>数量：${wmsInstock.num}</i>，&nbsp;&nbsp;<i>已入：${wmsInstock.realNum}</i>，&nbsp;&nbsp;<i>差异：${wmsInstock.num - wmsInstock.realNum}</i>，&nbsp;&nbsp;
                                    ...
                                    <div class="media-body">
                                    	<span class="pull-right">
											...
											<shiro:hasPermission name="wms:wmsOutstock:audit">
												<a href="${ctx}/wms/wmsOutstock/audit?id=${wmsOutstock.id}" onclick="return confirmx('确认要审核该出库单吗？', this.href)" class="btn btn-primary btn-sm" title="审核"><i class="fa fa-check"></i> 审核</a> 
											</shiro:hasPermission>
											</c:if>
                                        </span>
                                        <strong>单号：${wmsOutstock.no}</strong>，内容：${wmsOutstock.content}
                                        <p class="m-b-xs">
                                            <i>数量：${wmsOutstock.num}</i>，&nbsp;&nbsp;<i>已出：${wmsOutstock.realNum}</i>，&nbsp;&nbsp;<i>差异：${wmsOutstock.num - wmsOutstock.realNum}</i>，&nbsp;&nbsp;业务日期：<fmt:formatDate value="${wmsOutstock.dealDate}" pattern="yyyy-MM-dd"/>，&nbsp;&nbsp;
