	...
	<div class="">
		<div class="">
			<div class="ibox-header">
				<div class="pull-left">					
					<a class="btn btn-link" href="${ctx}/oa/oaCommonAudit/list">全部</a>
					<a class="btn btn-link" href="${ctx}/oa/oaCommonAudit/list?createBy.id=${fns:getUser().id}">我申请的</a>
					<a class="btn btn-link" href="${ctx}/oa/oaCommonAudit/list?currentBy.id=${fns:getUser().id}&status=1">待我审批的</a>
					<a class="btn btn-link" href="${ctx}/oa/oaCommonAudit/list?isSelfRead=1">抄送给我的</a>
					<a class="btn btn-link" href="${ctx}/oa/oaCommonAudit/list?status=1">审批中的</a>
					<a class="btn btn-link" href="${ctx}/oa/oaCommonAudit/list?status=2">已归档的</a>
					<a class="btn btn-link" href="${ctx}/oa/oaCommonAudit/list?beginCreateDate=${fns:getDate('yyyy-MM-dd')}&endCreateDate=${fns:getDayAfter(1)}">今日创建的</a>
					<a class="btn btn-link" href="${ctx}/oa/oaCommonAudit/list?beginCreateDate=${fns:getDayAfter(-7)}&endCreateDate=${fns:getDayAfter(1)}">7天内创建的</a>
					<a class="btn btn-link" href="${ctx}/oa/oaCommonAudit/list?beginCreateDate=${fns:getDayAfter(-30)}&endCreateDate=${fns:getDayAfter(1)}">30天内创建的</a>
				</div>
				<div class="pull-right">		
					...
					<a class="btn btn-white btn-sm" href="${ctx}/oa/oaCommonAudit/" title="刷新"><i class="fa fa-refresh"></i></a>
				...
				<div class="row">
					<div class="col-sm-12">
						<form:form id="searchForm" modelAttribute="oaCommonAudit" action="${ctx}/oa/oaCommonAudit/" method="post" class="form-inline">
							...
							<table:searchBox></table:searchBox><!-- 查询栏 -->
					...
					<c:forEach items="${page.list}" var="oaAudit">
						<tr>
							...
							<td>
								...
								<c:if test="${oaAudit.status == 1}"><span class="text-success">${fns:getDictLabel(oaAudit.status, 'common_audit_status', '')}</span></c:if>
								<c:if test="${oaAudit.status == 2}"><span class="text-primary">${fns:getDictLabel(oaAudit.status, 'common_audit_status', '')}</span></c:if>
							...
							<td>
								...
								<c:if test="${fns:getUser().id == oaAudit.createBy.id && oaAudit.status != 2}">
