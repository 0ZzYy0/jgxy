	...
	<div class="wrapper-content">
		<div class="ibox">
			...
			<sys:message content="${message}"/>
				...
				<div class="row">
					<div class="col-sm-12">
						<form:form id="searchForm" modelAttribute="testNote" action="${ctx}/test/note/testNote/selectList" method="post" class="form-inline">
				...
				<table id="contentTable" class="table table-bordered table-striped table-hover">
					...
					<c:forEach items="${page.list}" var="testNote">
						<tr>
							...
							<td class="codelabel">${testNote.createBy.id}</td>
							<td><fmt:formatDate value="${testNote.createDate}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
							<td>${testNote.remarks}</td>
							<td>${testNote.title}</td>
							<td>
								<shiro:hasPermission name="test:note:testNote:view">
									<a href="#" onclick="openDialogView('查看富文本测试', '${ctx}/test/note/testNote/view?id=${testNote.id}','800px', '500px')" class="btn btn-info btn-xs" title="查看"><i class="fa fa-search-plus"></i> 查看</a>
								</shiro:hasPermission>
							</td>
						</tr>
					</c:forEach>
