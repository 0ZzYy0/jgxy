		...
		<div class="col-sm-12 m-b-sm">
			<div class="pull-left">
				<shiro:hasPermission name="sys:dict:add">
					<table:addRow url="${ctx}/sys/dict/form" title="字典" pageModel="page"></table:addRow><!-- 增加按钮 -->
				</shiro:hasPermission>
				<shiro:hasPermission name="sys:dict:edit">
				    <table:editRow url="${ctx}/sys/dict/form" id="contentTable" title="字典" pageModel="page"></table:editRow><!-- 编辑按钮 -->
				</shiro:hasPermission>
				...
				<tr>
					...
					<td>
						<shiro:hasPermission name="sys:dict:view">
							<a href="${ctx}/sys/dict/form?id=${dict.id}" class="btn btn-info btn-xs" title="查看"><i class="fa fa-search-plus"></i></a>
						</shiro:hasPermission>
						<shiro:hasPermission name="sys:dict:edit">
	    					<a href="${ctx}/sys/dict/form?id=${dict.id}" class="btn btn-success btn-xs" title="修改"><i class="fa fa-edit"></i></a>
	    				</shiro:hasPermission>
	    				<shiro:hasPermission name="sys:dict:del">
							<a href="${ctx}/sys/dict/delete?id=${dict.id}&type=${dict.type}" onclick="return confirmx('确认要删除该字典吗？', this.href)" class="btn btn-danger btn-xs" title="删除"><i class="fa fa-trash"></i></a>
						</shiro:hasPermission>
						<shiro:hasPermission name="sys:dict:add">
							<a href="<c:url value='${fns:getAdminPath()}/sys/dict/form?type=${dict.type}&sort=${dict.sort+10}'><c:param name='description' value='${dict.description}'/></c:url>" class="btn btn-primary btn-xs" title="添加键值"><i class="fa fa-plus"></i></a>
						</shiro:hasPermission>
					</td>
				</tr>
			</c:forEach>
