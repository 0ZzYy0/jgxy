...
import com.javafast.common.persistence.BaseEntity;
...
//		System.out.println("dataScopeFilter: " + sqlString.toString());
	...
	public static void dataScopeFilter(BaseEntity<?> entity) {
		entity.getSqlMap().put("dsf", " AND a.account_id='"+UserUtils.getUser().getAccountId()+"'");
	}
	 ...
	 * @param entity
	 */
	public static void dataScopeFilterByRole(BaseEntity<?> entity) {

		if(entity.isApi()){
			entity.getSqlMap().put("dsf", " AND a.account_id='"+UserUtils.getUser()+"'");	
			return;
		}
		
		User user = UserUtils.getUser();

		// 数据范围（1：所有数据；5：所在部门数据；8：仅本人数据）
		StringBuilder sqlString = new StringBuilder();
		sqlString.append(" AND a.account_id='"+user.getAccountId()+"'");
		
		// 如果是超级管理员，则不过滤数据
		if (user.isAdmin()) {
			//设置到自定义SQL对象
			entity.getSqlMap().put("dsf", sqlString.toString());	
			return;
		}
		...
		if (Role.DATA_SCOPE_SELF.equals(dataScopeString)){
			...
			sqlString.append(" AND a.create_by='"+user.getId()+"'");
		}
		
		System.out.println("权限值："+dataScopeString+" 权限过滤追加sql:" + sqlString.toString());
	 ...
	 * @param entity
	 */
	public static void dataScopeFilterOwnBy(BaseEntity<?> entity) {
		...
		//5：所在部门数据
		if (Role.DATA_SCOPE_OFFICE.equals(dataScopeString)){
			
			sqlString.append(" AND a.office_id='"+user.getOffice().getId()+"'");
		}
		
		//8：仅本人数据
		if (Role.DATA_SCOPE_SELF.equals(dataScopeString)){
			
			sqlString.append(" AND (a.create_by='"+user.getId()+"' OR a.own_by='"+user.getId()+"')");
		}
